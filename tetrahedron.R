...
plot_rotations <- function(n=10) {
  vectors <- projection(random_rotation() %*% unit_tetra)
  for (i in 1:(n-1)) {
    vectors <- cbind(vectors, projection(random_rotation() %*% unit_tetra))
  }
...
# ggsave("test_tetra.png", units="px", width=2000, height=2000, bg="#2f2633", dpi="retina")

ggsave("test_tetra.png", units="px", width=2000, height=2000, bg="#FFFFFF", dpi="retina")
